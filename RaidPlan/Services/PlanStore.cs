...
public sealed class PlanStore
{
    ...
    private readonly Dictionary<string, RaidPlanDocument> plans = new();
    ...
    public RaidPlanDocument? Active { get; private set; }

    public IReadOnlyCollection<RaidPlanDocument> All => plans.Values;

    public IEnumerable<RaidPlanDocument> Ordered =>
    ...
    private void LoadAll()
    {
        foreach (var file in Directory.EnumerateFiles(directory, "*.json"))
        {
            try
            {
                ...
                var doc = JsonConvert.DeserializeObject<RaidPlanDocument>(json, Settings);
                if (doc == null)
                    continue;
        ...
        if (Active == null)
        {
            Active = RaidPlanDocument.CreateDefault("My first plan");
    ...
    public void SetActive(RaidPlanDocument plan)
    {
    ...
    public RaidPlanDocument CreateNew(string name)
    {
        var doc = RaidPlanDocument.CreateDefault(name);
        plans[doc.Id] = doc;
        Save(doc);
        SetActive(doc);
        return doc;
    ...
    public RaidPlanDocument Import(RaidPlanDocument doc, bool replaceExisting)
    {
    ...
    public void Delete(RaidPlanDocument doc)
    {
        plans.Remove(doc.Id);
    ...
    public void Save(RaidPlanDocument doc)
    {
        try
        {
    ...
    private string PathFor(RaidPlanDocument doc) => Path.Combine(directory, doc.Id + ".json");
}
